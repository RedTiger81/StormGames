...
class Schematic{
	...
	public function save(string $file) : void{
        $nbt = (new CompoundTag())
            ->setTag('Blocks', new ByteArrayTag($this->blocks))
            ->setTag('Data', new ByteArrayTag($this->data))
            ->setShort('Length', $this->length)
            ->setShort('Width', $this->width)
            ->setShort('Height', $this->height)
            ->setString('Materials', self::MATERIALS_POCKET);
        file_put_contents($file, (new BigEndianNbtSerializer())->writeCompressed(new TreeRoot($nbt, 'Schematic')));
	}
	...
	public function __construct(string $file){
		$nbt = (new BigEndianNbtSerializer())->readCompressed(file_get_contents($file))->getTag();
		...
		for($x = 0; $x < $this->width; $x++){
			for($z = 0; $z < $this->length; $z++){
				for($y = 0; $y < $this->height; $y++){
					...
					$block->getPos()->setComponents($x, $y, $z);
	...
	public function setBlocks(AxisAlignedBB $bb, \Generator $blocks) : void{
		...
		foreach($blocks as $block){
			$pos = $block->getPos()->subtract($offset);
	...
	protected function fixBlock(Block $block) : Block{
		...
		$fixed = BlockFactory::get($fix[0], $fix[1] ?? $block->getMeta());
		$fixed->getPos()->setComponents($block->getPos()->x, $block->getPos()->y, $block->getPos()->z);
		return $fixed;
	}
