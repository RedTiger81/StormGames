...
class Cuboid {
    ...
    public function set(ChunkManager $world, array $fullId, ?callable $callable = null) : void
    {
        $time = microtime(true);
        ...
        for ($x = $minX; $x <= $maxX; ++$x) {
            for ($z = $minZ; $z <= $maxZ; ++$z) {
                for ($y = $minY; $y <= $maxY; ++$y) {
                    ...
                    $iterator->currentSubChunk->setFullBlock($x & 0x0f, $y & 0x0f, $z & 0x0f, $fullId[mt_rand(-3, 0)] ?? $fullId[1]);
                }
            }
        }
